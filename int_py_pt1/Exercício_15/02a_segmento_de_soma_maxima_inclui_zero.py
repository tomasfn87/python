...
def main():
    ...
    while len(lista) < n_input:
        ...
        print('{0: ^2}'.format(cont_input),'-', end=' ') # 'print' permitem eliminar o espaço
                                       # vazio após '2.'.
        if n_input == 1:
            item_lista = lista.append(int(input(
            'insira o único inteiro da lista: '
                    )
                )
            )
        elif cont_input == n_input:
            item_lista = lista.append(int(input(
    ...
    c2 = 2 # e  'c2 = 1'. Nesse caso, o valor inicial de c2 foi determinado 
    ...
    if len(lista) == 1:
        ...
        print('\nA soma total dos elementos da lista é', soma_elementos(0, len(lista), lista), end='')
        # A linha acima informa para o usuário a soma de todos os
        # elementos da lista.
        ...
        while c1 != (len(lista) - 1): #and c2 != len(lista):
            while c2 <= len(lista):
                ...
                elif seg_soma == soma_elementos(c1, c2, lista):
                    if len(seg_soma_max) > len(lista[c1:c2]):
                        pass
                    else:
                        seg_soma_max = lista[c1:c2]
                        seg_soma = soma_elementos(c1, c2, lista)
        ...
        while c3 < len(lista):
            if seg_soma > lista[c3]:
                ...
                c3 += 1
            elif seg_soma == lista[c3] and seg_soma < 0:
                seg_soma = lista[c3]
                seg_soma_max = lista[c3:(c3 + 1)]
                c3 += 1
            elif seg_soma == lista[c3] and len(seg_soma_max) > 1:
                c3 += 1
        ...
        if seg_soma_max == lista:
            ...
            print(
                '.\nO maior segmento de soma continua sendo a soma total,',
                seg_soma, end=''
            )
        elif len(seg_soma_max) == 1:
            # Quando um elemento isolado  for maior do que a soma da lista toda:
            #     Quando apenas um elemento é positivo:
            #     - ou -
            #     Quando todos os elementos são negativos:
